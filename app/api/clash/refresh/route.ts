...
export async function POST() {
  ...
  for (const player of players) {
    ...
    const deaths = performances.reduce((s: number, p: any) => s + p.deaths, 0);
    const assists = performances.reduce((s: number, p: any) => s + p.assists, 0);
    const damage = performances.reduce(
      (s: number, p: any) => s + p.totalDamageDealtToChampions,
      0
    );
    const vision = performances.reduce((s: number, p: any) => s + p.visionScore, 0);
    ...
    data.push({
      ...
      clashScore: games > 0 ? Number((totalScore / recentMatches.length).toFixed(1)) : 0,
      recentMatches,
    });
  }

  data.sort((a, b) => b.clashScore - a.clashScore);
